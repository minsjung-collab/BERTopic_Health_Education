...
- `outputs/Topics_Over_Time.csv` — CSV transcription of the original May 24 temporal-topic workbook.
...
The `outputs/` directory contains CSV transcriptions of the original May 24 Excel workbooks for topic frequency, labels, temporal results, journal comparison, and year distribution. These preserve the worksheet cell values while remaining easy to inspect in GitHub and can be used to compare a reconstructed run with the historical results.
...
The unavailable historical `final_df.csv` means that a future PubMed retrieval cannot be guaranteed to reproduce the exact same 4,234 input records. The original execution log and CSV transcriptions of five archived output workbooks are provided to document and validate the historical run. The original `.xlsx` files are retained by the study author.
